...
bot.on('windowOpen', (window) => {
  console.log('🤖 Bot1: GUI opened, clicking first slot')
  setTimeout(() => {
...
let gotTitle = false
bot.on('title', (text) => {
  console.log('📺 Bot1 Title:', text)
  if (text.includes('Fight') || /^\d+$/.test(text.trim())) {
    gotTitle = true
    console.log('✅ Bot1: GOT EXPECTED TITLE')
  }
})

bot.on('subtitle', (text) => {
  console.log('Bot1 Subtitle:', text)
})

bot.on('spawn', () => {
  const items = bot.inventory.items()
  console.log('🤖 Bot1 Inventory:', items.map(item => item.name).join(', '))
  ...
  if (missing.length === 0 && missingArmor.length === 0) {
    console.log('✅ Bot1: GOT EXPECTED KIT')
  } else {
    console.log('❌ Bot1: FAILED TEST - Missing:', missing.concat(missingArmor).join(', '))
  }
  setTimeout(() => {
    if (!gotTitle) {
      console.log('❌ Bot1: FAILED TEST - No title received')
    }
  }, 15000)
})
