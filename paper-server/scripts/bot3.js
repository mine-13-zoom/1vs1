...
bot.on('windowOpen', (window) => {
  console.log('🤖 Bot3: GUI opened, clicking first slot')
  setTimeout(() => {
...
let gotTitle = false
bot.on('title', (text) => {
  console.log('📺 Bot3 Title:', text)
  if (text.includes('Fight') || /^\d+$/.test(text.trim())) {
    gotTitle = true
    console.log('✅ Bot3: GOT EXPECTED TITLE')
  }
})

bot.on('subtitle', (text) => {
  console.log('Bot3 Subtitle:', text)
})

bot.on('spawn', () => {
  const items = bot.inventory.items()
  console.log('🤖 Bot3 Inventory:', items.map(item => item.name).join(', '))
  ...
  if (missing.length === 0 && missingArmor.length === 0) {
    console.log('✅ Bot3: GOT EXPECTED KIT')
  } else {
    console.log('❌ Bot3: FAILED TEST - Missing:', missing.concat(missingArmor).join(', '))
  }
  setTimeout(() => {
    if (!gotTitle) {
      console.log('❌ Bot3: FAILED TEST - No title received')
    }
  }, 15000)
})
